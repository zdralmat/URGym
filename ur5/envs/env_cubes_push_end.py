...
class CubesPush(Env):
    ...
    def __init__(self, camera=None, reward_type='dense',  render_mode='human') -> None:
        ...
        # And one of the cubes position  (x, y, z)
        self.observation_space = Box(low=np.array([-1.0]*3 + [-math.pi]*4 + [-1.0]*3), high=np.array([1.0]*3 + [math.pi]*4 + [1.0]*3), dtype=np.float64)
        # Actions: x, y, z, qx, qy, qz, qw. 
        self.action_space = Box(low=np.array([-0.05]*3 + [-math.pi/10]*4), high=np.array([+0.05]*3 + [+math.pi/10]*4), dtype=np.float32)
    ...
    def get_observation(self):
        ...
        cube_position = self.get_cube_pose(self.cubes[1])[:3]
        obs = np.append(obs, cube_position)
        return obs
    ...
    def create_cubes(self):
        for id in self.cubes:
            p.removeBody(id)
        self.cubes = []
        self.create_cube(0.0, -0.3, 0.2)
        self.create_cube(0.0, -0.1, 0.1, [1,0,0,1])
