...
class LSTMActor(BasePolicy):
    """
    Actor network (policy) for SAC with an LSTM layer as the first layer.
    ...
    def __init__(
        ...
        # Define the LSTM layer as the first layer
        self.lstm = nn.LSTM(input_size=features_dim, hidden_size=net_arch[0], batch_first=True)
        last_layer_dim = net_arch[0]

        latent_pi_net = create_mlp(last_layer_dim, -1, net_arch[1:], activation_fn)
